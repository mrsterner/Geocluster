...
import dev.sterner.common.world.ReplaceOresFeature;
import dev.sterner.mixin.BiomeModificationContextImplMixin;
...
public interface LithosphereWorldgenRegistry {
    ...
    RegistryEntry<PlacedFeature> PLACED_DEPOSIT_FEATURE = PlacedFeatures.register("lithosphere:deposits_placed", CONFIGURED_DEPOSIT_FEATURE, placement);
    ...
    RegistryEntry<PlacedFeature> PLACED_REPLACED_ORES_FEATURE = PlacedFeatures.register("lithosphere:replace_ores_placed", CONFIGURED_REPLACED_ORES_FEATURE, placement);


    static void init(){

        BiomeModification modifications = BiomeModifications.create(Lithosphere.id("worldgen"));

        modifications.add(ModificationPhase.REMOVALS, BiomeSelectors.all(), ctx -> {
            Iterable<RegistryEntry<PlacedFeature>> registryEntries = getPlacedFeaturesByTag(ctx, LithosphereTagRegistry.ORES_TO_REMOVE);
    ...
    static RegistryEntry<ConfiguredFeature<DefaultFeatureConfig, ?>> registerConfigured(String id, Feature<DefaultFeatureConfig> feature) {
        return ConfiguredFeatures.register(Lithosphere.id(id).toString(), feature);
    }
